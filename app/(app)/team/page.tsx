  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        >
          {isShareholder ? "Summary mode" : "Assignment-first view"}
        </Badge>
      </div>

      {isShareholder ? (
        <div className="grid gap-4 xl:grid-cols-4">
          <SignalCard icon={UsersRound} label="Visible people" value={formatNumber(summary?.headcount ?? 0)} detail="Profiles in executive scope" />
          <SignalCard icon={FolderKanban} label="Active projects" value={formatNumber(summary?.activeProjects ?? 0)} detail="People currently engaged" />
          <SignalCard icon={Sparkles} label="Open tasks" value={formatNumber(summary?.activeTasks ?? 0)} detail="Visible execution load" />
          <SignalCard icon={CircleAlert} label="Needs attention" value={formatNumber(summary?.overloaded ?? 0)} detail="Pressure points to review" />
        </div>
      ) : (
        <>
          <div className="grid gap-4 xl:grid-cols-4">
            <SignalCard icon={UsersRound} label="Visible people" value={formatNumber(assignmentSummary.visiblePeople)} detail="People you can review now" />
            <SignalCard icon={Sparkles} label="Ready now" value={formatNumber(assignmentSummary.readyNow)} detail="Best candidates for new work" />
            <SignalCard icon={FolderKanban} label="In delivery" value={formatNumber(assignmentSummary.inDelivery)} detail="People already active on missions" />
            <SignalCard icon={CircleAlert} label="Needs attention" value={formatNumber(assignmentSummary.attention)} detail="Blocked or overdue pressure" />
          </div>
