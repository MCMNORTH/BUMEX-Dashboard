  ...
  const cards = [
    {
      ...
      detail: "Active versus total delivery streams",
    },
    {
      icon: PanelsTopLeft,
      label: "Open tickets",
      value: formatNumber(summary.openTickets),
      detail: `${formatNumber(summary.overdueTickets)} overdue in visible scope`,
    },
    {
      icon: ReceiptText,
      label: "Contracts",
      value: formatNumber(summary.activeContracts),
      detail: `${formatNumber(summary.expiredContracts)} expired contract${summary.expiredContracts === 1 ? "" : "s"}`,
    },
    {
      icon: FileStack,
      label: "Documents",
      value: formatNumber(summary.documentsCount),
      detail: `${formatNumber(summary.archivedDocumentsCount)} archived records`,
    },
  ];
        ...
        <Card className="border-border/70 bg-card/72 backdrop-blur-xl">
          <CardContent className="grid gap-3 px-4 py-4 sm:grid-cols-3">
            <div className="rounded-[16px] border border-border/65 bg-background/38 p-3">
              <p className="text-[10px] font-semibold tracking-[0.15em] text-muted-foreground uppercase">Last activity</p>
              <p className="mt-1.5 text-[13px] font-medium">{formatDate(summary.lastActivityDate)}</p>
            </div>
            <div className="rounded-[16px] border border-border/65 bg-background/38 p-3">
              <p className="text-[10px] font-semibold tracking-[0.15em] text-muted-foreground uppercase">Upcoming renewal</p>
              <p className="mt-1.5 text-[13px] font-medium">{summary.upcomingContractRenewal?.title ?? "None scheduled"}</p>
            ...
            <div className="rounded-[16px] border border-border/65 bg-background/38 p-3">
              <p className="text-[10px] font-semibold tracking-[0.15em] text-muted-foreground uppercase">Upcoming deadline</p>
              <p className="mt-1.5 text-[13px] font-medium">{summary.upcomingProjectDeadline?.name ?? "No project deadline"}</p>
        ...
        <Card className="border-border/70 bg-card/72 backdrop-blur-xl">
          <CardContent className="space-y-3 px-4 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-semibold tracking-[0.15em] text-muted-foreground uppercase">Relationship health</p>
                <h3 className="mt-1.5 text-base font-semibold tracking-tight">Client relationship signal</h3>
              </div>
              <RelationshipHealthBadge health={summary.relationshipHealth} />
            </div>
            <div className="grid gap-2.5 sm:grid-cols-3">
              <div className="rounded-[16px] border border-border/65 bg-background/38 p-3">
                <p className="text-[12px] font-medium">Delayed projects</p>
                <p className="mt-1.5 text-[1.45rem] font-semibold tracking-[-0.04em]">{formatNumber(summary.delayedProjects)}</p>
              </div>
              <div className="rounded-[16px] border border-border/65 bg-background/38 p-3">
                <p className="text-[12px] font-medium">Overdue tickets</p>
                <p className="mt-1.5 text-[1.45rem] font-semibold tracking-[-0.04em]">{formatNumber(summary.overdueTickets)}</p>
              </div>
              <div className="rounded-[16px] border border-border/65 bg-background/38 p-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[12px] font-medium">Contract risk</p>
