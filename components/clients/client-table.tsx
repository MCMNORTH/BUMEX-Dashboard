        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border/65 bg-background/35 text-xs uppercase tracking-[0.16em] text-muted-foreground">
            <tr>
              ...
              <th className="px-5 py-4 font-medium">Active projects</th>
              <th className="px-5 py-4 font-medium">Contract value</th>
              ...
              <tr key={client.id} className="border-b border-border/50 last:border-b-0">
                <td className="px-5 py-4">
                  <Link href={`/clients/${client.id}`} className="block">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">{client.industry ?? client.legal_name ?? "No descriptor"}</p>
                  </Link>
                </td>
                <td className="px-5 py-4"><ClientStatusBadge status={client.status} /></td>
                <td className="px-5 py-4">
                  <p>{client.accountManager?.full_name ?? "Unassigned"}</p>
                ...
                <td className="px-5 py-4">
                  <Badge variant="outline" className="rounded-full px-3 py-1">Placeholder</Badge>
                </td>
