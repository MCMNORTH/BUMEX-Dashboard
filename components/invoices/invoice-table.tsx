        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-border/65 bg-slate-50/80 text-[11px] uppercase tracking-[0.14em] text-slate-500 dark:bg-white/[0.03] dark:text-muted-foreground">
            <tr>
              ...
              <th className="px-5 py-4 font-medium">Gross amount</th>
              <th className="px-5 py-4 font-medium">Paid / Remaining</th>
              <th className="px-5 py-4 font-medium">Receipts</th>
            ...
            {invoices.map((invoice) => (
              <tr key={invoice.id} className="border-b border-border/50 transition-colors last:border-b-0 hover:bg-slate-50 dark:hover:bg-white/[0.03]">
                <td className="px-5 py-4">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{invoice.project?.name ?? "No linked project"}</p>
                </td>
                <td className="px-5 py-4"><InvoiceStatusBadge status={invoice.paymentStatus} /></td>
                <td className="px-5 py-4">{invoice.client?.name ?? "Not linked"}</td>
                ...
                <td className="px-5 py-4">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">{formatFinanceCurrency(invoice.remainingBalance, invoice.currency)} remaining</p>
                </td>
